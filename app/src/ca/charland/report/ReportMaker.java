		...
		for (Point p : pts) {
			if (p.isHighlighted()) {
				...
				if(Type.StatutoryHoliday.equals(highlight.type)) {
					...
					reporter.setItalizedString(p.highlight.description, x + 1, y++);
				} else if(Type.Birthday.equals(highlight.type)) {
					reporter.setUnderlineString(p.value, p.x, p.y);
					reporter.setUnderlineString(p.highlight.description, x + 1, y++);
				}
				else {
					reporter.setBoldedString(p.value, p.x, p.y);
					reporter.setBoldedString(p.highlight.description, x + 1, y++);
				}
